...
// Coordinates providers, registry, executor, Python Forge, Task Runner and workspace.
...
export class AgentOrchestrator extends EventEmitter {
  ...
  constructor(_apiKey: string, options: { registry?: ToolRegistry; executor?: ToolExecutor } = {}) {
    ...
    const registry = options.registry ?? createToolRegistry(path.join(userData, 'tool_registry.db'), path.join(this.projectRoot, 'tools.json'));
    ...
    try {
      ...
      logger.error('[agent_orchestrator] initialization failed', { error: err instanceof Error ? err.message : String(err) });
    }
  }
  ...
  private queueAutomationTask(kind: string, prompt: string): string {
    ...
    try {
      interactionLedger.insertInteraction({ uuid: taskId, timestamp_epoch: Date.now(), interaction_type: 'automation_trigger', raw_transcript_input: prompt, model_response_output: `Queued ${kind} task`, context_snapshot_json: JSON.stringify({ taskKind: kind, status: 'queued' }), embedding_vector_id: `v_${taskId}`, performance_latency_ms: 0 });
  ...
  private rebuildDeclarations(): void {
    ...
    try { geminiLiveBridge.setToolDeclarations(this.toolDeclarations); }
    catch (err) { logger.warn('[agent_orchestrator] failed to push tool declarations', { error: err instanceof Error ? err.message : String(err) }); }
  }
  ...
  public async handleToolCall(toolCall: any): Promise<FunctionResponsePayload[]> {
    ...
    for (const fc of toolCall.functionCalls) {
      ...
      try {
        ...
        functionResponses.push({ id, name, response: result.success ? { result: result.payload } : { error: result.error ?? 'Tool execution failed' } });
  ...
  public getToolRegistry(): Map<string, IToolDefinition> {
    ...
    for (const tool of this.registry.list()) map.set(tool.id, { id: tool.id, name: tool.name, description: tool.description, sourceCode: tool.sourceCode, compiledFn: null, status: tool.status === 'active' || tool.status === 'compiled' ? 'compiled' : 'failed', createdAt: tool.createdAt, permissions: tool.permissions });
    return map;
  }
  ...
  /** Existing deterministic plans first; when no plan matches, automatically Forge a real Python capability. */
  public async runTask(request: string): Promise<TaskTrace> {
    const trace = await this.taskRunner.runTask(request);
    if (trace.status !== 'failed' || !/No built-in capability matched/i.test(trace.summary)) return trace;
    try {
      const built = await this.builder.ensureCapability(request);
      const fallback: TaskTrace = {
  ...
  private resolveSandboxedPath(requested: string): string {
    if (typeof requested !== 'string' || requested.trim().length === 0) throw new Error('A non-empty relative path is required for sandboxed file operations.');
    const resolved = path.resolve(this.projectRoot, requested); const rootWithSep = this.projectRoot.endsWith(path.sep) ? this.projectRoot : this.projectRoot + path.sep;
    if (resolved !== this.projectRoot && !resolved.startsWith(rootWithSep)) throw new Error(`Path "${requested}" escapes the agent project sandbox.`);
    return resolved;
  }

  private resolveHostPath(requested: string): string {
    const raw = String(requested ?? '').trim(); if (!raw) throw new Error('A non-empty host path is required.');
    const profile = process.env.USERPROFILE ?? process.env.HOME ?? ''; if (!profile) throw new Error('User profile path is unavailable.');
    const roots = [path.join(profile, 'Desktop'), path.join(profile, 'Documents'), path.join(profile, 'Downloads')].map(p => path.resolve(p));
    const namedFolder = raw.match(/^(desktop|downloads?|documents?)[\\/]*(.*)$/i);
    const base = namedFolder ? roots[/^desktop$/i.test(namedFolder[1]) ? 0 : /^downloads?$/i.test(namedFolder[1]) ? 2 : 1] : roots[0];
    const relative = namedFolder ? namedFolder[2] : raw; const resolved = path.resolve(path.isAbsolute(raw) ? raw : path.join(base, relative));
    if (!roots.some(root => resolved === root || resolved.startsWith(root + path.sep))) throw new Error('Host path must be inside Desktop, Documents, or Downloads.');
    return resolved;
  }

  public async shutdown(): Promise<void> {
    try { pythonRuntime.stopWorker(); } catch (err) { logger.error('[agent_orchestrator] python worker stop failed', { error: err instanceof Error ? err.message : String(err) }); }
    try { this.executor.shutdown(); } catch (err) { logger.error('[agent_orchestrator] sandbox worker stop failed', { error: err instanceof Error ? err.message : String(err) }); }
    try { this.registry.close(); } catch (err) { logger.error('[agent_orchestrator] registry close failed', { error: err instanceof Error ? err.message : String(err) }); }
  }
}
